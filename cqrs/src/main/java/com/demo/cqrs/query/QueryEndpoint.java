...
public class QueryEndpoint {
    ...
    private final Map<Class<?>, QueryFunction<Request, Response>> functionMap = new HashMap<>();

    public QueryEndpoint(ApplicationContext applicationContext) {
        for (QueryFunction<Request, Response> function : applicationContext.getBeansOfType(QueryFunction.class).values()) {
            Class<?>[] generics = GenericTypeResolver.resolveTypeArguments(ClassUtils.getUserClass(function), QueryFunction.class);

            if (generics != null) {
                functionMap.put(generics[0], function);
                log.info("initialize QueryFunction: {}", ClassUtils.getUserClass(function));
            }
        }
    }
    ...
    private Message<Response> call(Message<Request> requestMessage) {
        ...
        QueryFunction<Request, Response> function = functionMap.get(request.getClass());
        ...
        try {
            Response response = function.query(request);
